...
//		jQuery('#ls-layers .ls-layer-box').eq( jQuery(el).index() ).find('.ls-sublayers td:first').click();
    ...
    // get selected
    var selected_index = draggable.children('.ls-selected').index() || index;
    ...
    if (draggable[0])
      jQuery(draggable[0].children[selected_index]).addClass('ls-selected');
	...
	save : function(el) {
		...
		setTimeout(function() {
			...
			jQuery('#ls-layers .ls-layer-box').each(function(layer) {
				...
				var $bg = $data.filter('[name$="[background]"]');
				$bg.val($bg.val().replace(/^http(s?):\/\//, "http$1//"));
              ...
              if (id.match(/DOCTYPE/i)) {
                // session expired
                if (!window.backendWnd) {
                  alert("Your session has expired!\n"+
                    "Please login in the following popup window, to save your slider.");
                  backendWnd = window.open(document.location.href.split('?')[0]+"?option=com_layer_slider&view=autologin", "backend",
                    "width=630,height=434,screenX="+(screen.width/2 - 315)+",screenY="+(screen.height/2 - 217));
                  backendWnd.focus();
                  backendTmr = setInterval(function() {
                    if (backendWnd && backendWnd.closed) {
                      clearInterval(backendTmr);
                      window.backendWnd = 0;
                      jQuery('.ls-publish').removeClass('saving').find('button').text('Save changes').removeAttr('disabled');
...
jQuery(document).ready(function() {
	...
	// Global
		...
		// CodeMirror
		if(document.location.href.indexOf('&view=slider') === -1) {
			LS_CodeMirror.init();
		}
