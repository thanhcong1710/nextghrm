...
class ModKiwiAgentRegisterHelper {

        public static function getAjax() {
                // Get module parameters
                jimport('joomla.application.module.helper');
                $input = JFactory::getApplication()->input;
                $module = JModuleHelper::getModule('kiwiagentregister');
                ...
                if ($cmd = $input->get('cmd')) {
                        ...
                        // Verify post data
                        if (empty($data['rp_name']) || empty($data['rp_email']) || empty($data['rp_subject']) || empty($data['rp_message']) || empty($data['rp_agent_type'])) {
        ...
        protected static function createEmail($template, $data) {
                if (empty($template)) {
                        ...
                        $html = str_replace('{visitor_phone}', $data['rp_phone'], $html);
                        $html = str_replace('{visitor_address}', $data['rp_address'], $html);
                        $html = str_replace('{visitor_agent_type}', $data['rp_agent_type'], $html);
