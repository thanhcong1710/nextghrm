...
if (!in_array($position, array('main-menu', 'home-menu', 'footer-nav', 'bottom-2', 'bottom-3'))) {
        $dataToggle = ($item->parent && $params->get('showAllChildren')) ? 'data-toggle="dropdown"' : '';
} else {
        $dataToggle = '';
}

$icon = ($item->parent && $item->level == 1 && $params->get('showAllChildren') && $module->position != 'bottom-2' && $module->position != 'bottom-3') ? '<b class="caret"></b>' : '';
