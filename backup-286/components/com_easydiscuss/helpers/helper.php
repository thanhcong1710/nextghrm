...
class DiscussHelper
{
        ...
        public static function getUnansweredCount($categoryId = '0', $excludeFeatured = false)
        {
                ...
                if ($categoryIds)
                {
                        if (count($categoryIds) == 1)
                        {
                                ...
                                $query->where('a.category_id = ' . (int) $categoryIds);
                        } else
                        {
                                $query->where('a.category_id IN (' . implode(',', $categoryIds) . ')');
                        }
                }
        ...
        /**
         * Method count all featured post
...
/*]]>*/";
        ...
        public static function getSimilarQuestion($text = '')
        {
                ...
                if (!$config->get('main_similartopic_privatepost', 0))
                {
                        ...
                        {
                                $queryExclude = 'a.category_id NOT IN (' . implode(',', $excludeCats) . ')';
                                $query->where($queryExclude);
                        }
                }
                ...
                $query->select('id')
                        ->from('#__discuss_tags')
                        ->where('MATCH(' . $db->quoteName('title') . ') AGAINST (' . $db->quote($tagkeywords) . ' IN BOOLEAN MODE)')
                        ->where('language IN (' . $db->quote($languageTag) . ',' . $db->quote('*') . ')');
                $db->setQuery($query);
                $tagResults = $db->loadAssocList(null, 'id');
                ...
                if (count($tagResults) > 0)
                {
                        ...
                        $tagQuery->select('a.id, a.title, MATCH(a.title, a.content) AGAINST (' . $db->quote($text) . ' WITH QUERY EXPANSION) AS score')
                                ->from('#__discuss_posts AS a')
                                ->innerJoin('#__discuss_posts_tags AS b ON a.id = b.post_id')
                                ->where('MATCH(a.title, a.content) AGAINST (' . $db->quote($text) . ' WITH QUERY EXPANSION)');
                        $tagQuery->where('a.published = 1')
                                ->where('a.parent_id = 0')
                                ->where('b.tag_id IN (' . implode(',', $tagResults) . ')')
                                ->where('a.language in (' . $db->quote($languageTag) . ',' . $db->quote('*') . ')');

                        if ($queryExclude)
                        {
                                $tagQuery->where($queryExclude);
                        }

                        $query->union($tagQuery);
                }
...
//end function setMeta
        ...
        public static function getLanguageField($name, $value, $client = 'site')
        {
                $options = JLanguageHelper::createLanguageList($value, constant('JPATH_' . strtoupper($client)), true, true);
