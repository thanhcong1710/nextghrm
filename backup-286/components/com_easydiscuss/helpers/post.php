...
class DiscussPostHelper
{
        ...
        public static function isVoted($postId, $user_id = null)
        {
                ...
                $query->select('COUNT(d.id) AS isVoted')
                        ->from('#__discuss_posts AS a')
                        ->leftJoin('#__discuss_votes AS d ON a.id = d.post_id')
                        ->where('a.id = ' . $db->quote($postId))
                        ->where('a.language IN (' . $db->quote(JFactory::getLanguage()->getTag()) . ',' . $db->quote('*') . ')')
                        ->where('d.user_id = ' . $db->quote($my->id));
                $db->setQuery($query);
        ...
        {
                if (isset(self::$_load[$attachmentId]))
                {
                        return self::$_load[$attachmentId];
                }
                ...
                $query->select('p.user_id, p.id AS post_id, p.parent_id AS parent_id')
                        ...
                        ->leftJoin('#__discuss_posts AS p ON p.id = a.uid')
                ...
                {
                        self::$_load[$attachmentId] = $result;
                }
                return $result;
        }
